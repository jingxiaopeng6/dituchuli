...
export const FORMAT_CATALOG = [
  ...
  },
  {
    ...
    lossy: false,
    backendOnly: true,
    notes: 'COG 需要分块、金字塔和布局约束，建议后端 GDAL 生成。',
  },
  {
    id: 'jp2',
    label: 'JPEG2000',
    ...
    lossy: true,
    backendOnly: true,
    notes: '浏览器不适合编码 JP2，建议后端或桌面 GDAL。',
  },
  {
    id: 'img',
    label: 'ERDAS IMG',
...
function cloneTypedArray(input) {
...
function getSourceFormat(source) {
  ...
  if (kind === 'dem') return ext === 'img' ? 'img' : 'geotiff'
  if (kind === 'imagery') {
...
function toRgbaBytes(source) {
  ...
  if (record?.kind === 'imagery') {
    if (record?.channels && record.channels >= 4 && sourceData.length >= pixelCount * 4) {
      return sourceData instanceof Uint8ClampedArray ? cloneTypedArray(sourceData) : new Uint8ClampedArray(sourceData)
    }
...
function toGreyscaleBytes(source) {
  const record = getSourceRecord(source)
  const { width, height } = getDimensions(record)
  const pixelCount = width * height
  const sourceData = record?.data
  ...
  for (let i = 0, p = 0; i < pixelCount; i += 1, p += 4) {
    let value = Number(sourceData?.[i] ?? 0)
    if (record?.kind === 'dem') {
      value = ((value - minElevation) / range) * 255
    } else {
      value = value * 255
    }
...
function drawSourceToCanvas(source) {
  ...
  if (record?.kind === 'imagery' && record?.channels === 3) {
    const rgba = toRgbaBytes(record)
    ctx.putImageData(new ImageData(rgba, width, height), 0, 0)
    return canvas
  }

  const rgba = toGreyscaleBytes(record)
  ctx.putImageData(new ImageData(rgba, width, height), 0, 0)
  return canvas
}
...
function buildGeoMetadata(source, targetKind) {
  ...
  if (bbox) {
    ...
    if (epsg) {
      ...
      metadata.GeographicTypeGeoKey = 4326
      warnings.push('未识别 CRS，GeoTIFF 将按 EPSG:4326 写入 GeoKey；请在 GIS 中复核。')
    }
  } else {
...
export function describeRoute(source, targetFormat) {
  ...
  const targetIsBackendOnly = !!target?.backendOnly
  ...
  if (targetIsBackendOnly) {
    warnings.push(`${target.label} 需要后端 GDAL/桌面工具支持，当前浏览器仅提供路线提示。`)
    return {
      sourceFormat,
      targetFormat,
      route: 'backend',
...
export function buildCompatibilityMatrix() {
  return FORMAT_CATALOG.map((item) => ({
    ...item,
    status: item.backendOnly ? '后端支持' : '浏览器支持',
  }))
}
...
export function buildImportedDatasetFromConversion(result, source) {
  if (!result?.importedData) return null
  const record = result.importedData
  const sourceName = record?.sourceFile || source?.sourceFile || source?.name || 'raster'
  return {
    ...record,
