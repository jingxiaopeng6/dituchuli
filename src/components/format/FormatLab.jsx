    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          <Pill tone={route.supported ? 'good' : route.route === 'backend' ? 'warn' : 'bad'}>
            {route.supported ? '浏览器可转' : route.route === 'backend' ? '需后端' : '受限'}
          </Pill>
  ...
  return (
    <div className="rounded-2xl border border-white/6 bg-slate-950/30 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <Pill tone={item.backendOnly ? 'warn' : item.browserWritable ? 'good' : 'bad'}>
          {item.backendOnly ? '后端' : item.browserWritable ? '浏览器' : '不可用'}
        </Pill>
      </div>
    ...
    >
      <div>
        ...
        <p className="mt-1 text-[11px] leading-5 text-terrain-muted">
          这里把 GeoTIFF 作为核心中转，浏览器内可完成 GeoTIFF / ENVI / PNG / JPEG / BMP 的真实导出；COG、JP2、IMG、NITF 等保持为后端路线提示。
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="rounded-xl border border-white/6 bg-slate-950/35 px-3 py-2">
          <div className="text-[10px] text-slate-400">可浏览导出</div>
      ...
      <Card
        title="1. 选择源数据"
        desc="可以直接从当前工作区里的 DEM / 影像 / 合成结果发起转换。"
      >
      ...
      <Card
        title="2. 选择目标格式"
        desc={targetDefinition?.notes || '选择一个输出格式，系统会自动告诉你当前路径是否需要后端。'}
      >
        <div className="grid grid-cols-2 gap-2">
          {availableTargets.map((item) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-medium text-terrain-text">{item.label}</div>
                <Pill tone={item.backendOnly ? 'warn' : 'good'}>{item.backendOnly ? '后端' : '浏览器'}</Pill>
              </div>
              <div className="mt-1 text-[11px] leading-5 text-terrain-muted">{item.notes}</div>
              <div className="mt-2 flex flex-wrap gap-2">
                <Pill tone="neutral">{item.extension}</Pill>
      ...
      <Card title="3. 路线判断" desc="系统会根据源数据和目标格式给出真实可执行路径。">
        <div className="space-y-2 text-[11px] leading-5 text-terrain-muted">
          <div className="flex flex-wrap gap-2">
            <Pill tone={route?.supported ? 'good' : route?.route === 'backend' ? 'warn' : 'bad'}>
              {route?.supported ? '浏览器可直接转换' : route?.route === 'backend' ? '需要后端 GDAL' : '当前路径受限'}
            </Pill>
      ...
      <Card title="4. 执行转换" desc="导出后可以选择自动回写到当前工作区，方便继续分析和叠加。">
      ...
      <Card title="能力矩阵" desc="这些是当前项目内和后端路线的边界，避免误以为所有格式都能在浏览器里双向互转。">
